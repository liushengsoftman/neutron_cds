...
class TunnelRpcCallbackMixin(object):
    ...
    def tunnel_sync(self, rpc_context, **kwargs):
        ...
        if driver:
            ...
            #    roamed to a different IP. Delete any reference to the new
            #    local_ip or the host id. Don't notify tunnel_delete for the
            #    old IP since that one could have been taken by a different
            #    agent host-id (neutron-ovs-cleanup should be used for that).
            #    Finally create a new endpoimt for the (tunnel_ip, host).
            if host:
                ...
                if (ip_endpoint and ip_endpoint.host is None
                    ...
                    LOG.info(
                        _LI("Tunnel IP %(ip)s in use with host %(host)s, "
                            "this is OK if the host name was roamed from IP "
                            "to IP"),
